...
function LinkedInOAuthCallbackContent() {
  ...
  useEffect(() => {
    const handleCallback = async () => {
      ...
      if (state && !validateStateParameter(state, 'linkedin')) {
        const errorMessage = {
          type: 'OAUTH_ERROR',
      ...
      if (error) {
        ...
        const errorMessage = {
          type: 'OAUTH_ERROR',
          error: `LinkedIn OAuth error: ${error}${errorDescription ? ' - ' + errorDescription : ''}`
        };
      ...
      try {
        ...
        const message = {
          type: 'OAUTH_SUCCESS',
          userEmail: 'linkedin-user@example.com', // Would be real user email in production
          provider: 'linkedin',
